...
# MAGIC %md
# MAGIC ### **Custom PySpark Data-Source: Write to Sharepoint**
...
!pip install azure-identity msgraph-sdk nest-asyncio
...
# MAGIC %md
# MAGIC #### Implement Sharepoint Resources
# MAGIC - Currently supports Sharepoint Lists
# MAGIC - Derive from `Resource` to implement further resources (e.g. file-types excel, pdf, etc.)
...
# MAGIC %md
# MAGIC #### Implement WriterCommitMessage, DataSourceStreamWriter & DataSource
...
fields = json.dumps(          # <-- make sure to create your Sharepoint List according to the field-mapping here!
    {
        "name": "Name",
        "industry": "Industry",
...
list_data = streaming_df.select(                    # <-- If your Sharepoint List looks different, update the processing to match the schema and the field-mapping above!
    col("value").cast("string").alias("name"),
    lit("Technology").alias("industry"),
    (col("value") * 100000).cast("double").alias("annual_revenue")
)
...
query = (
    list_data.writeStream
      .format("pyspark.datasource.sharepoint")
      .option("pyspark.datasource.sharepoint.auth.tenant_id", tenant_id)
      .option("pyspark.datasource.sharepoint.auth.client_id", client_id)
      .option("pyspark.datasource.sharepoint.auth.client_secret", client_secret)
      .option("pyspark.datasource.sharepoint.site_id", site_id)
      .option("pyspark.datasource.sharepoint.resource", resource)
      .option("pyspark.datasource.sharepoint.list.list_id", list_id)
      .option("pyspark.datasource.sharepoint.list.fields", fields)
      .option("pyspark.datasource.sharepoint.batch_size", batch_size)
      .option("pyspark.datasource.sharepoint.fail_fast", "true")
